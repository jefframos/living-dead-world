...
export default class WeaponInGameView extends BaseComponent {
    ...
    setData(weapon) {
        this.weapon = weapon;
        console.log(this.weapon.weaponViewData)
    ...
    update(delta) {
        ...
        this.spriteList.forEach(element => {
            ...
            element.sprite.x = Math.cos(element.angle) * 20 + this.weapon.weaponViewData.baseViewData.viewOffset.x
            element.sprite.y = Math.sin(element.angle) * 20 + this.weapon.weaponViewData.baseViewData.viewOffset.y
        });
    }
    destroy() {
        super.destroy();
